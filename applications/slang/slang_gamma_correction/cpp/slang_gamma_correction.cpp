...
class SourceOp : public Operator {
 public:
  ...
  void setup(OperatorSpec& spec) override {
    spec.param(allocator_,
               ...
               std::static_pointer_cast<Allocator>(
                   fragment()->make_resource<RMMAllocator>("allocator")));
  ...
  void compute(InputContext& op_input, OutputContext& op_output,
    ...
    tensor->reshape<uint8_t>(shape_, nvidia::gxf::MemoryStorageType::kHost, gxf_allocator.value());

    std::memcpy(tensor->pointer(), data_.data(), data_.size());
...
class SlangGammaCorrectionApp : public holoscan::Application {
 public:
  void compose() override {
    ...
    auto gamma_correction =
        make_operator<ops::GammaCorrectionOp>("GammaCorrection",
                                              Arg("gamma", 2.2f),
...
int main() {
  auto app = holoscan::make_application<SlangGammaCorrectionApp>();
  app->run();
