 ...
 public:
  ...
  void compute(InputContext& op_input, OutputContext& op_output,
    ...
    video_buffer->resize<nvidia::gxf::VideoFormat::GXF_VIDEO_FORMAT_RGBA>(
        2, 3, nvidia::gxf::SurfaceLayout::GXF_SURFACE_LAYOUT_PITCH_LINEAR,
        nvidia::gxf::MemoryStorageType::kHost, gxf_allocator.value(), false /* stride_align */);
...
class SlangShaderApp : public holoscan::Application {
 public:
  ...
  void add_source_op() { add_source_op_ = true; }
  ...
  void add_sink_op() { add_sink_op_ = true; }
  ...
  void compose() override {
    ...
    if (add_source_op_) {
      auto source_op = make_operator<SourceOp>("source_op");
    ...
    if (!add_source_op_ && !add_sink_op_) {
      add_operator(op);
    }
  }
  ...
  bool add_source_op_ = false;  ///< Flag to enable source operator in pipeline
  bool add_video_buffer_source_op_ =
      false;                  ///< Flag to enable video buffer source operator in pipeline
  bool add_sink_op_ = false;  ///< Flag to enable sink operator in pipeline
};
...
TEST(SlangShaderAppTest, ParameterStridesOfVideoBuffer) {
  ...
  application->check_output("8 4 32\n");
}
