...
get_reference_specs(CommandWorkspace& workspace, const std::string& reference_name,
  ...
  if (maybe_reference_tensor) {
    ...
    if (maybe_reference_video_buffer) {
      ...
      strides.resize(shape.rank());
      strides[0] = static_cast<uint64_t>(color_plane.bytes_per_pixel) * element_size;
      strides[1] = static_cast<uint64_t>(color_plane.stride);
      strides[2] = strides[1] * static_cast<uint64_t>(color_plane.height);
...
void CommandStrideOf::execute(CommandWorkspace& workspace) {
  ...
  if (reference_shape.rank() > 1) {
    strides.x = reference_strides[reference_shape.rank() - 2];
    if (reference_shape.rank() > 2) {
      strides.y = reference_strides[reference_shape.rank() - 3];
      if (reference_shape.rank() > 3) {
        strides.z = reference_strides[reference_shape.rank() - 4];
        if (reference_shape.rank() > 4) {
          throw std::runtime_error(fmt::format(
              "Attribute 'strides_of': Only tensors of rank <= 4 are supported, found rank '{}'.",
              reference_shape.rank()));
        }
      } else {
        strides.z = strides.y * strides.x;
      }
    } else {
      strides.y = strides.x;
    }
...
void CommandReceiveCudaStream::execute(CommandWorkspace& workspace) {
  ...
  // stream
  if (workspace.op_input_.inputs().size() > 0) {
